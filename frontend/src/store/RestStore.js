...
class RestStore {
  ...
  @action
  getOrganisations() {
    if (this._organisationsListTimer) return;
    const handler = async () => {
    ...
    handler();
    this._organisationsListTimer = setInterval(handler, 30000);
  }
  ...
  @computed get organisationsNamesList() {
    return this._organisationsList.map(org => org.login);
  }
  ...
  getOrganisationsEvents() {
    ...
    handler();
    this._organisationsEventsTimer = setInterval(handler, 30000);
  }
  ...
  @computed get addressPoints() {
    return this.organisationsEvents.map(({ lat, long, title }) => ({
      lat,
      long,
      title
    }));
